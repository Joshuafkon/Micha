...
void loop()
{
  ...
  switch (state) {
    ...
    case GO_FORWARD:
      ...
      digitalWrite(dir_1, LOW);    //controls the direction the motor HIGH = Forward
      digitalWrite(dir_2, LOW);    //controls the direction the motor HIGH = Forward
      analogWrite(pwm_2, 110);        //increase the speed of the motor from 0 to 255
      analogWrite(pwm_1, 110);        //decrease the speed of the motor from 255 to 0;    POWER SENT TO RIGHT TRACK MUST BE ~75% POWER BECASUE OF HIGH FRICTION ON LEFT SIDE.
      nextState = READ_LINE;
      break;
    case GO_LEFT:
